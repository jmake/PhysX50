...
std::vector< float* > UnityVerts; 
std::vector< int > UnitynVerts; 

std::vector< int* > UnityIndices; 
std::vector< int > UnitynIndices; 
...
0002 [UnityInit] triVerts:550 triangles:832
		"[UnityInit] triVerts:" + std::to_string( n1 / 3 ) +
		" triangles:" + std::to_string( n2 / 3 ) 

0029 [createDeformableVolume] gDeformableVolumes:3 triVerts:1719 triIndices:960
...
void UnityClear()
{
//    std::cout<<"[SpicyX] UnityClear"<<std::endl;
...
{
	//spicyX.MeshAdd(vertices, vertices.Length, triangles, triangles.Length); 
	UnitynVerts.push_back( n1 ); 
	UnityVerts.push_back( outArray1 ); 

	UnitynIndices.push_back( n2 ); 
	UnityIndices.push_back( outArray2 ); 
...
void UnityCreate(
	 PxArray<PxVec3>& unityVerts,  
	 PxArray<PxU32>& unityIndices, 
	...
	) 
{
	...
	//PxArray<PxVec3> unityVerts; 
	unityVerts.clear(); 
	unityVerts.resize(nVertices); // n1 -> nVertices

	for(int i=0,k=0; i < nVertices; i++)
	{
		unityVerts[i] = PxVec3(outArray1[nDims*i+0], outArray1[nDims*i+1], outArray1[nDims*i+2]) ; 
		printf("%d) %f %f %f \n", i, unityVerts[i][0], unityVerts[i][1], unityVerts[i][2]);
	}
	...
	//PxArray<PxU32> unityIndices; 
	unityIndices.clear(); 
	unityIndices.resize(n2);
	for (int i = 0; i < n2; ++i) 
	{
		unityIndices[i] = static_cast<PxU32>(outArray2[i]);
		printf("%d) %d \n", i, unityIndices[i]);
	}

	logger->log(
		"[UnityCreate] nVertices:" +  
		std::to_string( nVertices ) + " (" + std::to_string( unityVerts.size() ) + ")"
		" nTriangles:" + 
		std::to_string( nTriangles ) + " (" + std::to_string( unityIndices.size() / nIndices ) + ")"
	);

	logger->error(nVertices != unityVerts.size(), "[ERROR] nVertices != unityVerts.size() !!"); 
	logger->error(nTriangles != unityIndices.size() / 3, "[ERROR] nTriangles != unityIndices.size() / 3 !!"); 
}
...
static void createDeformableVolumes(const PxCookingParams& params)
{
	...
	// MeshCreate 
	printf("[createDeformableVolumes] nVertices:%zd nTriangles:%zd \n", UnityVerts.size(), UnityIndices.size());
	for(int i=0; i < UnityVerts.size(); i++)
	{
		PxArray<PxVec3> unityVerts;   
		PxArray<PxU32> unityIndices; 
		UnityCreate(
			unityVerts, unityIndices, 
			UnityVerts[i], UnitynVerts[i], 
			UnityIndices[i], UnitynIndices[i]); 

		PxRemeshingExt::limitMaxEdgeLength(unityIndices, unityVerts, maxEdgeLength);
		createDeformableVolume(params, unityVerts, unityIndices);
	}
...
void CleanupPhysics(bool /*interactive*/)
{
    ...
    std::cout<<"[SpicyX] cleanupPhysics2"<<std::endl;

	releaseScene(); //PX_RELEASE(gScene);
    std::cout<<"[SpicyX] cleanupPhysics3"<<std::endl;

	PX_RELEASE(gDispatcher);
	PX_RELEASE(gPhysics);
    std::cout<<"[SpicyX] cleanupPhysics4"<<std::endl;
	if (gPvd)
	{
    ...
    std::cout<<"[SpicyX] cleanupPhysics5"<<std::endl;

	PxCloseExtensions();
    std::cout<<"[SpicyX] cleanupPhysics6"<<std::endl;

	PX_RELEASE(gCudaContextManager);
    std::cout<<"[SpicyX] cleanupPhysics7"<<std::endl;
...
int SpicyTest()
{
	...
	static const PxU32 frameCount = 69;
...
void renderActors(PxRigidActor** actors, const PxU32 numActors, int itime) 
{
	...
	for(PxU32 i=0,idx=0; i<numActors; i++)
	{
		...
		std::cout<<"[SpicyX] iActors:" << i <<" nbShapes:"<< nbShapes << std::endl;
  	}
...
//	StepPhysics(interactive, iteration);
	...
	if(nbActors)
	{
		...
		renderActors(&actors[0], static_cast<PxU32>(actors.size()), iteration);
	}

	//gSkinning->synchronize(); // ?? 

	std::cout<<"[SpicyX] nDeformableVolumes:" << gDeformableVolumes.size() <<std::endl;
	for (PxU32 i = 0; i < gDeformableVolumes.size(); i++)
	{
		DeformableVolume* dv = &gDeformableVolumes[i];
		renderDeformableVolume(dv->mDeformableVolume, dv->mPositionsInvMass, iteration);
	}
