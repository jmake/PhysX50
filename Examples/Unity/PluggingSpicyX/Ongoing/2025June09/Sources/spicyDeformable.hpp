...
void UnitySoftInit(float* outArray1, int n1, int* outArray2, int n2) 
{
...
void UnitySoftPositionSet(float x, float y, float z)
{
	if(rbkObj == NULL) return; 

	rbkObj->PositionSet(x, y, z); 
}


void UnitySoftPositionGet(float& x, float& y, float& z)
{
	if(rbkObj == NULL) return; 

	rbkObj->PositionGet(x, y, z); 
}
