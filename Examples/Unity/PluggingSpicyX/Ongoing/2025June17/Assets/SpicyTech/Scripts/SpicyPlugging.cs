...
public class SpicyPlugging : MonoBehaviour
{
    ...
    void RigidBodiesEvolve(SpicyX spicyX) 
    {
        if(kinematic)
        {
            ...
            {
                rb.GameObject2GlobalPose(spicyX); 
            }
        }
        else
        {
            foreach (SpicyRigidBody rb in rigidBodies)
            {
                rb.GlobalPose2GameObject(spicyX); 
            }
        }
    } 
...
*/
    ...
    void DeformablesInit(SpicyX spicyX) 
    {
        foreach (GameObject softObject in softObjects)
        {
            Debug.Log("[DeformablesInit] " + softObject.name);
    ...
    )
    {
        ...
        if (!mesh.isReadable)
        {
            Debug.LogError("[MeshCreate] ERROR!! Mesh '"+ obj.name +"' is not readable " + 
            "(Model -> Inspector -> Import Settings -> Meshes -> Read/Write)");
            return;
        }
