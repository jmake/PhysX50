...
std::string TransformShow(PxTransform com) 
{
    PxVec3 p = com.p; 
    PxQuat q = com.q;
    ...
    parts.push_back("Pos: (" + std::to_string(p.x) + ", " + std::to_string(p.y) + ", " + std::to_string(p.z) + ")");
    parts.push_back("Rot: (" + std::to_string(q.x) + ", " + std::to_string(q.y) + ", " + std::to_string(q.z) + ", " + std::to_string(q.w) + ")");

    std::ostringstream oss;
    for (const std::string& part : parts) oss << part << " | ";

    std::string msg = oss.str();
    if (!msg.empty()) msg.erase(msg.size() - 3); 
    return msg;
}
...
public:
	...
	{
		targetPose = GlobalPoseCreate(px, py, pz, qx, qy, qz, qw); // eKINEMATIC 
	}	
	...
	void GeometrySet(PxTriangleMeshGeometry mesh, PxTransform transform) 
	{
		...
		if(eKINEMATIC)
		{
			...
			newPose = transform; // initial position 
		} 
    ...
    {
        newPose = PxTransform(PxVec3(x, y, z)); // eKINEMATIC 
    }
    ...
    int Update(int iteration, PxReal deltaTime, Logger* logger)
    {
		...
		if(eKINEMATIC) 
		{
			logger->log("[UnityHard] targetPose("+ std::to_string(iteration) +") " + TransformShow(targetPose) ); 
			rigidBody->setKinematicTarget(targetPose);
		}
...
void 
UnityHardUpdate(int iteration, PxReal deltaTime, Logger* logger) 
{
	...
	{
		body.Update(iteration, deltaTime, logger);
	}
