...
{
    std::cout<<"[SpicyX] stepPhysics ("<< iteration <<")"<<std::endl;
	...
	{
//std::cout<<"[SpicyX] DeformableVolume ("<< i <<")"<<std::endl;

		DeformableVolume* dv = &gDeformableVolumes[i];
		std::vector<float> flatArray = dv->copyDeformedVerticesFromGPU(iteration, i);
std::cout<<"[SpicyX] "
<<"iteration:"<< iteration <<" "
<<"ibody:"<<  i <<" "
<<"nbVertices:"<< flatArray.size() / 4 <<" "
<< std::endl;
/*
...
int SpicyTest()
{
	...
	int i=-1;
	for (i = 0; i < frameCount; i++) StepPhysics(false, i);
